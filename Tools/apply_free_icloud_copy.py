...
def replace_expected(rel: str, old: str, new: str, expected: int = 1) -> None:
    path = root / rel
    text = path.read_text(encoding="utf-8")
    count = text.count(old)
    if count != expected:
        raise SystemExit(f"{rel}: expected {expected} match(es), found {count}: {old[:100]!r}")
...
replace_expected("index.html", "<h2>Lifetime dual backup</h2>", "<h2>iCloud backup &amp; Lifetime</h2>")
replace_expected(
    "index.html",
...
replace_expected("index.html", "<h2>雙重備份永久解鎖</h2>", "<h2>免費 iCloud 備份與 Lifetime</h2>")
replace_expected(
    "index.html",
...
replace_expected(
    "privacy/index.html",
    "Asympta 私隱政策：本機優先、可選 iCloud + Google Drive 雙重備份，並且不含廣告或追蹤。",
    "Asympta 私隱政策：本機優先、免費 iCloud 備份、可選 Lifetime Google Drive 備份，並且不含廣告或追蹤。",
)
replace_expected(
    "privacy/index.html",
